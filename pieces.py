            ...
            for j in range(8):
                if self.ar[i][j] != '  ':
                    ...
                    if self.ar[i][j] == '..' or self.ar[i][j] == '...':
                        pygame.draw.rect(
                                self.screen, CLR,
                                (PIECE_SIZE*(j), PIECE_SIZE*(i), PIECE_SIZE, PIECE_SIZE))
                    elif self.ar[i][j][0] == '.':
                        pygame.draw.rect(
                                self.screen, CLR,
                                (PIECE_SIZE*(j), PIECE_SIZE*(i), PIECE_SIZE, PIECE_SIZE))
                        self.P[self.ar[i][j][1:]].rect = cal_rect(1, i,j)
                        self.screen.blit(self.P[self.ar[i][j][1:]].surf, cal_rect(1, i,j))
                    else:
                        self.P[self.ar[i][j]].rect = cal_rect(1, i,j)
                        self.screen.blit(self.P[self.ar[i][j]].surf, cal_rect(1, i,j))
